...
3. Runs **fully offline**. There is no telemetry and no network egress. Client files never leave the machine.
...
- **Canvas limit.** If any scaled dimension exceeds 5765 mm (227 in), fail with the hint: "Set `scale: 0.1` for this format in the registry."
...
- No outbound network calls at all. A test asserts this by monkeypatching `socket.create_connection`.
- **Path safety.**
