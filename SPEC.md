...
- 2026-09-26 — Photoshop artboards all came out the default size: `makeArtboard()` nested `artboardRect` under "using", where Photoshop ignores it. The rect now goes at the top level of the Make descriptor (the ScriptListener shape). The script then reads the artboard back with `executeActionGet` and, if the size is still wrong, forces it with `editArtboardEvent` (`changeSizes: 1`). Any artboard still the wrong size shows up as a warning in the UI instead of failing silently. `export_digital.jsx` now crops to the artboard rect rather than `layer.bounds`, which is the artwork's extent, not the artboard's. Illustrator artboard names were right but only appeared after opening Artboard Options, so the script now calls `app.redraw()`. All of this is unverified on a real machine.
- 2026-09-26 — Print export type now follows `exports` first. The Illustrator export used to pick PDF or TIFF only from the *scaled* artboard size, ignoring `exports`, so a `[tiff]`-only format drawn at 1:10 (the IDEA scroller) and a `[pdf]`-only format over 1000mm (the Paradise frame) both silently exported nothing. Python now decides the print deliverable per artboard when the job is created: the only allowed type, or, when both PDF and TIFF are listed, the physical-size threshold. `preflight_export.jsx` exports exactly the PDF/TIFF deliverables the job lists (JSX stays thin). Also fixed the TIFF validator expecting `physical × tiff_ppi / scale` pixels (10× too many at 1:10), not the scaled document's actual `physical × tiff_ppi`. Formats may now override `tiff_ppi` and `tiff_if_longest_side_mm_over`; the scroller uses `tiff_ppi: 30`, the supplier's 300 dpi on the 1:10 file. Old installs: the relaunch script only removed `<old>.previous` under the old version's own name, so every update left one more folder behind. On startup the app now deletes older `StudioHelper-v*` folders next to it that contain the launcher, keeping only the newest older one as the rollback copy and never touching newer versions. Added a Guide page covering the end-to-end flow.
- 2026-09-26 — "Check & export print" on an untouched demo file exported nothing and didn't say why clearly. Export only runs when every check passes, so this part was by design. Fixed around it: (1) a new `artboard-empty` check says plainly that an artboard has nothing on it (and replaces the noisier bleed-coverage fail for that artboard); (2) the job page now leads with a one-line summary (exported N / nothing exported, N problems / stopped with an error), lists problems first, folds the passed checks away, and shows script-level errors, which were previously dropped entirely when the JSX threw; (3) bleed is now scaled with the artwork everywhere -- creation (`effectiveBleedMm`), preflight, PDF/TIFF export, and the PDF/TIFF validators. Previously the document got the full bleed while preflight expected the scaled one, so every 1:10 artboard failed `bleed-size`, and the PDF validator ignored scale entirely. (4) The registry's `StudioHelper_X1a` preset was never shipped (see 2026-09-23), so PDF export now falls back to Illustrator's built-in `[PDF/X-1a:2001]` with a visible warning, not a hard failure. Print files created before this change for 1:10 formats still carry the old full bleed and will fail `bleed-size`; recreate them (Start revision).
- 2026-09-26 — UI redesign (v0.4.0), built from a clickable mockup she approved. Reason: the marketing director's chats showed the real process. Sizes and bleed come from the print shop per job, mall print is always TIFF (PDF only for small print), and nobody records the specs. The old catalogue-only New job page and the flat list of 36 checkboxes didn't fit that. The pages are now Jobs (progress per job), New job (formats grouped by the new registry `group` field, presets, search, per-job bleed override on each ticked format, "+ Custom size" with optional "save to Formats"), Job (4 steps: set up → design → check & export → delivered; the file list is the centre of the page; details, check results and the manual-script fallback open in a side panel), and Formats (grouped, with the new per-format `notes` for print-shop requirements; "+ Add format" writes to registry.yaml). Guide, Setup check, updates and Quit moved behind a "?" menu. Palette and type follow Figma (Inter, #0d99ff, neutral greys) because that's the tool she lives in. Inter and JetBrains Mono (OFL, licences in LICENSES/) are bundled as woff2 because the app is offline. Saving a format edits registry.yaml as text, inserting at the end of `formats:`, so her comments survive (a PyYAML round trip would drop them); the file is re-validated and restored if the result doesn't load. Custom sizes in a job are all validated before any is saved. Digital formats listing several exports ([jpg, png], [jpg, mp4]) now accept any one of them (`collapse_alternatives`): Photoshop writes only one, so the others used to stay "missing" forever and such a job could never read as delivered. Mall print formats in the default registry are `exports: [tiff]`. Verified by driving the real server with seeded jobs and validator fixtures through headless Chromium over the DevTools protocol (light, dark, 390 px phone width, drawers, custom size → create job end to end). Headless Chromium on this machine needs `--disable-dev-shm-usage --no-zygote` to not hang.
- 2026-09-26 — First real run of "Check & export print" failed with "TypeError: undefined is not an object". Cause: `preflight_export.jsx` read `doc.documentBleedOffsetRect`, which only exists on `DocumentPreset`, not on an open Document, so every matched artboard threw. (Most likely also why the 0.3.6 demo exported nothing: the old UI dropped script errors.) The bleed-size check now runs only when the bleed is readable, otherwise it's skipped. Output bleed doesn't depend on it, because the PDF export sets it explicitly and the TIFF export expands the artboard by it. That explicit PDF bleed also used non-existent `bleedTop/bleedBottom/...` options; it now uses the documented `bleedOffsetRect` with `bleedLink = false`. Script exceptions from both Illustrator and Photoshop now include the file and line (`SH.describeError`), so a photo of the message is enough to find the cause.
- 2026-09-26 — First real exports (v0.4.1), three findings. (1) BleedBox came out 215.6×302.6 mm for a 210×297 A4 with 3 mm bleed: exactly 8 pt a side, i.e. Illustrator truncated 3 mm (8.5 pt) to whole points. The scripts now ask for bleed rounded *up* to whole points (`SH.bleedPt`: 3 mm → 9 pt = 3.18 mm), and the PDF/TIFF validators accept anything from the exact bleed up to that round-up (`match.doc_bleed_range_mm`), never less. (2) Bleed coverage ("nothing reaches the bleed") is now a warning in both the Illustrator preflight and the PDF validator: a design whose edge is meant to stay white paper is legitimate, and it used to block export. Empty artboards still block. (3) Photoshop exported an "Instagram post" at 1212×1692: the export used whatever size the artboard had. It now measures each artboard (and the cropped result) against the format and refuses to write a wrong-size file, naming the size it found, and the job page shows those warnings. The root cause of the wrong-size artboard itself is still unconfirmed. PDF preset: the planned `StudioHelper_X1a` .joboptions was never made (it needed the print department's settings, SPEC §13), and the director's chats show there isn't one house standard: print shops give specs per job, mall print is TIFF. The default is now Illustrator's built-in `[PDF/X-1a:2001]`, and Setup check verifies whatever preset the registry names, explaining the fallback instead of pointing at SPEC.md.
